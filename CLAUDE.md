...
**Price freshness**: scrape.py stamps each result with `scraped_at` (UTC ISO-8601). rank.py surfaces this as a "Prices captured: …" line below the picks table. Amazon prices move — sale items can shift hourly, stable SKUs weekly. Re-scrape within 24h of any purchase decision. The `scraped_at` field is on the scrape output (results.json), **not** in `nutrition_data.json` — nutrition facts are static, so don't conflate the two. When a price entry lacks `scraped_at` (legacy results.json or fixtures), `rank()` backfills with the current UTC datetime so the freshness line still renders — an approximation, but ranking happens at consumption time so "now-ish" is the most honest signal available for unstamped data.
