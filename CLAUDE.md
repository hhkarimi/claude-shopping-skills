...
**Price history archive**: `data/history/<skill>-<YYYY-MM-DD>.json` holds snapshots of past scrape runs (file format = scrape.py output, one file per skill per day). Useful for tracking price trends over time, comparing the same product across dates, and re-running rankings from a past snapshot via `rank.py --prices data/history/powders-2026-05-27.json`. Files accumulate as new scrapes happen — long-term, this directory may be compressed; for now we accept the bloat.
